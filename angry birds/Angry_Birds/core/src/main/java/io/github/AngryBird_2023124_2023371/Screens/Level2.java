...
public class Level2 implements Screen,Serializable{
   ...
   private void setupGameObjects() {
       ...
       // Keep the bird setup from Level1.java
       for (int i = 0; i < 3; i++) {
       ...
       float leftTowerX = WORLD_WIDTH * 0.6f-10f-20f-40f-20f;
       blocks.addAll(Arrays.asList(
           new WoodenBlock(new Vector2(leftTowerX, 80f), world),
           new WoodenBlock(new Vector2(leftTowerX, 130f), world),
           new WoodenBlock2(new Vector2(leftTowerX, 180f), world)
       ));

       // Middle tower (3 blocks high)
       float middleTowerX = WORLD_WIDTH * 0.75f-90f-20f-40f;
       blocks.addAll(Arrays.asList(
           new WoodenBlock(new Vector2(middleTowerX, 80f), world),
           new WoodenBlock2(new Vector2(middleTowerX, 130f), world)
       ));

       // Create square formation
       float rightTowerX = WORLD_WIDTH * 0.2f+100f+200f+70f;
       float startY = 80f;
       float blockSize = 50f;
       
       // Add blocks in a 4x4 grid to form a square
       for (int row = 1; row <= 4; row++) {
           for (int col = 1; col <= 4; col++) {
               // Skip the blocks in the middle for a hollow square
               if ((row == 1 && col == 2) || (row == 2 && col == 3) || 
                   (row == 2 && col == 2) || (row == 1 && col == 3)) {
                   continue;
               }
               float x = rightTowerX + col * blockSize;
               float y = startY + row * blockSize;
               blocks.add(new WoodenBlock(new Vector2(x, y), world));
           }
       }

       // Add pigs with new positions
       BasePig leftPig = new BigPig(atlas, new Vector2(rightTowerX+70f, 380f), world);
       leftPig.setScale(PIG_SCALE);
       pigs.add(leftPig);
       stage.addActor(leftPig);
       
       BasePig rightPig = new BigPig(atlas, new Vector2(rightTowerX+150f, 380f), world);
       rightPig.setScale(PIG_SCALE);
       pigs.add(rightPig);
       stage.addActor(rightPig);
       
       BasePig centrePig = new BigPig(atlas, new Vector2(rightTowerX+220f, 380f), world);
       centrePig.setScale(PIG_SCALE);
       pigs.add(centrePig);
       stage.addActor(centrePig);
   ...
   private void createUI() {
       ...
       @Override
       public void clicked(InputEvent event, float x, float y) {
           game.setScreen(new SaveLoadMenu(game, Level2.this));
       }
       });
   ...
   stage.addActor(saveLoadButton);
       ...
       pauseButton.addListener(new ClickListener() {
           ...
           public void clicked(InputEvent event, float x, float y) {
               setPaused(true);
               saveGameToJson(SaveSlot.SLOT_2);
               game.setScreen(new PauseMenuScreen(game, Level2.this));
           }
       });
   ...
   public static Level2 loadGame(AngryBirds game, String saveSlot) {
       try {
           ...
           Level2 loadedLevel = new Level2(game);
...
public static Level2 loadGameFromJson(AngryBirds game, String saveSlot) {
   try {
       FileHandle file = Gdx.files.local(saveSlot);
       if (!file.exists()) {
       ...
       for (JsonPig jsonPig : state.pigs) {
           // Use BigPig instead of MechanicPig
           BasePig pig = new BigPig(loadedLevel.atlas,
       ...
       for (JsonBlock jsonBlock : state.blocks) {
           BaseBlock block;
           // Create the correct block type based on the saved type
           switch(jsonBlock.type) {
               case "WoodenBlock":
                   block = new WoodenBlock(
                       new Vector2(jsonBlock.x * PPM, jsonBlock.y * PPM),
                       loadedLevel.world);
                   break;
               case "WoodenBlock2":
                   block = new WoodenBlock2(
                       new Vector2(jsonBlock.x * PPM, jsonBlock.y * PPM),
                       loadedLevel.world);
                   break;
               default:
                   // Fallback to WoodenBlock if type is unknown
                   block = new WoodenBlock(
                       new Vector2(jsonBlock.x * PPM, jsonBlock.y * PPM),
                       loadedLevel.world);
                   break;
