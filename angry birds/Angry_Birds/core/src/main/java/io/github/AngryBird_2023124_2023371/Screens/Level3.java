...
public class Level3 implements Screen,Serializable{
   ...
   private void setupGameObjects() {
       ...
       // Block and pig setup from dummy.java
   ...
   private void createUI() {
       ...
       @Override
       public void clicked(InputEvent event, float x, float y) {
           game.setScreen(new SaveLoadMenu(game, Level3.this));
       }
       });
   ...
   stage.addActor(saveLoadButton);
       ...
       pauseButton.addListener(new ClickListener() {
           ...
           public void clicked(InputEvent event, float x, float y) {
               setPaused(true);
               saveGameToJson(SaveSlot.SLOT_3);
               game.setScreen(new PauseMenuScreen(game, Level3.this));
           }
       });
   ...
   public static Level3 loadGame(AngryBirds game, String saveSlot) {
       try {
           ...
           Level3 loadedLevel = new Level3(game);
...
public static Level3 loadGameFromJson(AngryBirds game, String saveSlot) {
   try {
       FileHandle file = Gdx.files.local(saveSlot);
       if (!file.exists()) {
       ...
       for (JsonPig jsonPig : state.pigs) {
           BasePig pig = new AngryPig(loadedLevel.world,loadedLevel.atlas,new Vector2(jsonPig.x * PPM+20.0f, jsonPig.y * PPM+20.f));
